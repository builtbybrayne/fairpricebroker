	...
	<section class="verticals" aria-label="Your instruments">
		{#each data.verticals as v (v.id)}
			<article class="panel vertical" class:vertical--soon={!v.ready}>
				...
				{#if v.id === 'recruiting'}
					<p class="panel__lede">
						The salary conversation without the standoff: the client's budget once, a private link
						per candidate, and a fair salary with a straight read on the overlap.
					</p>
					<p class="vertical__glance">
						...
						{:else}
							{data.roleCount}
							{data.roleCount === 1 ? 'role' : 'roles'} · {data.openCount} with a candidate still to answer
						{/if}
					</p>
					<div class="vertical__actions">
						<a class="pill pill--navy btn" href={resolve('/app/roles')}>Open roles</a>
						<a class="pill pill--gold btn" href={resolve('/app/new')} data-testid="new-role"
							>New role</a
						>
					</div>
				{:else}
