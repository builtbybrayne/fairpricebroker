<script lang="ts">
	...
	const questions = $derived(salaryNegotiationTemplate.questions[data.side]);
	const rows = $derived(questions.map((q) => ({ key: q.key, label: q.label, help: q.prompt })));
	const symbol = $derived(symbolFor(data.currency));
	// The seat that sent the link: the broker in this vertical.
	const senderName = $derived(data.terms.broker.toLowerCase());
	...
	// What to do now, by outcome: the sender has the same read.
	const nextStep = $derived(
		!result
			? ''
			: result.zone === 'comfort'
				? `The ${senderName} sees the same result. Expect to hear from them about the role; salary should not be the sticking point, so use the conversation for everything else that matters to you.`
				: result.zone === 'deal'
					? `The ${senderName} sees the same result. There is a deal here but it is a stretch, so when they get in touch, be ready to talk about what beyond salary would make it work for you.`
					: `The ${senderName} sees the same gap. Unless this role appeals for reasons beyond salary, it may not be worth pursuing hard; if it does, say so when they get in touch.`
	);
	const axis = $derived(
		result
...
<main class="shell shell--narrow side">
	{#if data.phase === 'enter'}
		...
		{#if data.showDisclosure}
			<section class="panel disclosure" aria-labelledby="disclosure-title" data-testid="disclosure">
				<h2 id="disclosure-title" class="panel__title">
					{salaryNegotiationTemplate.disclosure.heading}
				</h2>
				<p class="disclosure__body">{salaryNegotiationTemplate.disclosure.body}</p>
				<h2 class="panel__title disclosure__incentive-title">
					{salaryNegotiationTemplate.incentive.heading}
				</h2>
				<p class="disclosure__body" data-testid="incentive">
					{salaryNegotiationTemplate.incentive.body}
				</p>
			</section>
		{/if}
	...
	{:else if data.phase === 'sealed'}
		...
		<section class="panel next" aria-labelledby="next-title">
			...
			<p class="panel__lede">
				When the other side's figures are also in, the fair salary is worked out privately. You will
	...
	{:else if data.phase === 'cancelled'}
		<section class="panel" aria-labelledby="cancel-title">
			...
			<p class="panel__lede">
				The {senderName} withdrew it before a result was produced. Nothing you entered has been shared.
			</p>
		</section>
	{:else if data.phase === 'closed' && result && ownRange}
		...
		<section class="panel" aria-labelledby="guidance-title">
			...
			<p class="guidance" data-testid="side-copy">{result.guidance.sideCopy}</p>
